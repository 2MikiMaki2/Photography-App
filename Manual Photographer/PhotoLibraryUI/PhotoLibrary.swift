...
class PhotoLibrary {
    ...
    static var temp = "temp not changed"
    ...
    static func getPhotos() -> [AlbumPhoto] {
        if (albumLocation == "") {
            ...
            PHPhotoLibrary.shared().performChanges {
                ...
                temp = "temp has changed"
                print("placeholder localIdentifier: " + assetPlaceholder.localIdentifier)
                print("albumLocation: " + albumLocation)
                print(temp)
            } completionHandler: { success, error in
                if let error {
                    print("Error saving photo: \(error.localizedDescription)")
